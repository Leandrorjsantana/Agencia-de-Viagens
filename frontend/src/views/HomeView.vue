<template>
  <div v-if="pageData">
    ...
    <!-- Promoções -->
    <div class="post-hero-sections">
      <div class="container">
        <PromotionCards 
          v-if="pageData.promotion_cards && pageData.promotion_cards.length > 0" 
          :cards="pageData.promotion_cards" 
        />
      </div>
    </div>
...
import PromotionCards from '../components/PromotionCards.vue';

export default {
  name: 'HomeView',
  components: { Carousel, Slide, Pagination, Navigation, OfferCard, PromotionCards },
  ...
  data() { 
    return { 
      ...
      carouselSettings: {
        itemsToShow: 1.2,
        snapAlign: 'start',
        breakpoints: { 600: { itemsToShow: 2.2 }, 900: { itemsToShow: 3.2 }, 1200: { itemsToShow: 4 } }
      },
      formData: {},
  ...
  watch: { 
    activeTab() { this.formData = {}; this.updateFormContainerHeight(); },
    pageData: {
      immediate: true,
      handler(newData) {
        if (newData?.services?.length > 0 && !this.activeTab) { this.activeTab = newData.services[0].slug; }
  ...
  computed: { 
    roomsDisplayText() { 
      ...
      if (totalChildren > 0) { text += `, ${totalChildren} Criança${totalChildren > 1 ? 's' : ''}`; }
    ...
    heroStyle() {
      ...
      if (config?.hero_background_image) {
        return { backgroundImage: `linear-gradient(rgba(0,0,0,0.4),rgba(0,0,0,0.4)),url(${config.hero_background_image})` };
      }
    ...
    newsletterStyle() { 
      ...
      if (config?.newsletter_background_image) {
        return { backgroundImage: `url(${config.newsletter_background_image})`, backgroundSize: 'contain', backgroundRepeat: 'no-repeat', backgroundColor: '#f4f5f7' };
      }
  ...
  methods: {
    ...
    handleSearchSubmit() { 
      ...
      for (const key in this.formData) { if (this.formData[key]) { queryParams.append(key, this.formData[key]); } }
      if (serviceSlug === 'hospedagens') { queryParams.append('rooms', JSON.stringify(this.rooms)); }
    ...
    async handleNewsletterSubmit() {
      try {
        ...
        if (response.status === 201) { alert(this.pageData.site_configuration.newsletter_success_message || "Inscrição realizada com sucesso!"); this.newsletterEmail = ''; }
      } catch (error) { alert("Ocorreu um erro ao se inscrever."); }
    },
    getAutoplaySpeed(speed) { const speedMs = Number(speed); return !isNaN(speedMs) && speedMs > 0 ? speedMs : 0; },
  },
};
</script>
...
.services-bar button:hover, .services-bar button.active {
  background-color: var(--primary-color);
  color: #fff;
}

.services-bar button:hover i, .services-bar button.active i {
  color: #fff;
}
...
.newsletter-content h2 { font-size: 2rem; margin-bottom: 8px; }
.newsletter-content p { font-size: 1.1rem; margin-bottom: 15px; }
